...
MIKA_r_time= /* ランダム練習 累積練習時間 秒 */
	...
	MIKA_k_pos1_c1="-^\\"; /* キーボード 最上段 シフト無し記号 文字列 */
	MIKA_z_pos1_c1="－＾￥"; /* キーボード 最上段 シフト無し記号 全角文字列 */
	MIKA_k_pos1_c2="!\"#$%&'() =~|"; /* キーボード最上段 シフト有り記号 表示文字列 */
	MIKA_k_pos1_h2="!\"#$%&'()=~|"; /* キーボード最上段 シフト有り記号 練習文字列 */
	...
	MIKA_k_pos4_c2="<>?_"; /* キーボード 下一段 シフト有り記号 文字列 */
	MIKA_z_pos4_c2="＜＞？＿"; /* キーボード 下一段 シフト有り記号 全角文字列 */
	MIKA_c_pos4=MIKA_a_pos4+MIKA_k_pos4_c1; /* キーボード 下一段刻文字列 */
	MIKA_s_pos4=MIKA_a_pos4+MIKA_k_pos4_c2; /* キーボード 下一段刻文字列 */
	MIKA_c_post=[MIKA_c_pos1,MIKA_c_pos2,MIKA_c_pos3,MIKA_c_pos4]; /* キーボード シフト無し 刻印文字列テーブル */
	MIKA_s_post=[MIKA_s_pos1,MIKA_s_pos2,MIKA_s_pos3,MIKA_s_pos4]; /* キーボード シフト有り 刻印文字列テーブル */
	MIKA_h_pos1=MIKA_c_pos3+MIKA_k_pos3_c1+MIKA_k_pos3_c2+MIKA_k_pos3_c2; /* ホームポジション 練習文字列 */
	MIKA_h_pos2=MIKA_c_pos2+MIKA_k_pos2_c1+MIKA_k_pos2_c2+MIKA_k_pos2_c2; /* 上一段 練習文字列 */
	MIKA_h_pos3=MIKA_h_pos1+MIKA_h_pos2; /* ホームポジション＋上一段 練習文字列 */
	MIKA_h_pos4=MIKA_a_pos4+MIKA_k_pos4_h1+MIKA_k_pos4_h1+MIKA_k_pos4_c2+MIKA_k_pos4_c2; /* 下一段 練習文字列 */
	MIKA_h_pos5=MIKA_h_pos1+MIKA_h_pos4; /* ホームポジション＋下一段 練習文字列 */
	MIKA_h_pos6=MIKA_h_pos1+MIKA_h_pos2+MIKA_h_pos4; /* ホームポジション＋上一段＋下一段 練習文字列 */
	MIKA_h_pos7=MIKA_a_pos1+MIKA_k_pos1_c1+MIKA_k_pos1_c1+MIKA_k_pos1_h2+MIKA_k_pos1_h2; /* 最上段 練習文字列 */
	MIKA_h_pos8=MIKA_h_pos1+MIKA_h_pos2+MIKA_h_pos4+MIKA_h_pos7; /* 全段  練習文字列 */
	...
	MIKA_h_table=MIKA_k_pos1_c1+MIKA_k_pos1_c2+MIKA_k_pos2_c1+MIKA_k_pos2_c2+MIKA_k_pos3_c1+MIKA_k_pos3_c2+MIKA_k_pos4_h1+MIKA_k_pos4_c2; /* 半角記号文字テーブル */
	MIKA_z_table=MIKA_z_pos1_c1+MIKA_z_pos1_c2+MIKA_z_pos2_c1+MIKA_z_pos2_c2+MIKA_z_pos3_c1+MIKA_z_pos3_c2+MIKA_z_pos4_h1+MIKA_z_pos4_c2; /* 全角記号文字テーブル */
...
MIKA_text_underline=2; /* ランダム練習の下線表示位置 */
...
MIKA_capital_char=MIKA_a_pos3.charAt(0); /* エラー文字 小文字大文字変換用 基準文字 */
MIKA_en_mark1=String.fromCharCode(0xa5); /* chrome book での \ コード */
MIKA_en_mark2='\\'; /* windows での \ コード */
...
MIKA_menu_mes=[ /* ポジション練習 ランダム練習 メニュー項目 */
	...
	MIKA_menu_cord=[ /* ポジション練習 ランダム練習 メニュー項目表示位置 x座標 y座標 */
		[2*14,20*8],
		[4*14,20*8],
		[6*14,20*8],
		[8*14,20*8],
...
addEventListener( "keyup", keyupfunction ); /* キーアップ処理追加 */
window.onload = function() {
 const MIKAKIGO = document.getElementById("MIKAKIGO"); /* MIKAKIGO キャンバス取得 */
    if (MIKAKIGO.getContext) { /* MIKAKIGO キャンバスが存在した場合 */
      const g = MIKAKIGO.getContext("2d");//2次元描画
		MIKA_win_size_width=MIKAKIGO.width; /* 表示ウィンドー横サイズ取得 */
		MIKA_win_size_height=MIKAKIGO.height; /* 表示ウィンド縦サイズ取得 */
		readcookie(); /* クッキー読み込み */
...
function keydownfunction(event) /* キーダウン処理 */
{
	var nChar;
	const MIKAKIGO = document.getElementById("MIKAKIGO"); /* MIKAKIGO キャンバス取得 */
 	if (MIKAKIGO.getContext) { /* MIKAKIGOのキャンバスが存在した場合 */
    	const g = MIKAKIGO.getContext("2d"); /* 2次元描画 */
		...
		else if(nChar==MIKA_en_mark1) nChar=MIKA_en_mark2;	/* Chromebookで入力コードが\の場合はバックスラッシュコードに変換する */	
...
function keyupfunction(event) /* キーダウン処理 */
{
	var nChar;
	const MIKAKIGO = document.getElementById("MIKAKIGO"); /* MIKAKIGO キャンバス取得 */
 	if (MIKAKIGO.getContext) { /* MIKAKIGOのキャンバスが存在した場合 */
...
function cslputscale(g,x,y,mes,scale) /* 仮想座標から実座標に変換して文字列を指定の倍率で表示 */
{
	var i,j,length;
	...
	for(i=0;i<length;i++) 
	{
		cslputscale2(g,x,y+j,mes[i],scale); /* 指定位置に一文字表示 */
		j=j+8*scale*charlength(mes[i]); /* 表示文字位置更新 半角文字は y座標を 1 加算 全角文字は 2加算 */
	}
}
function cslputscale2(g,x1,y1,mes,scale) /* 仮想座標から実座標に変換して文字列を指定の倍率で一文字づつ表示 */
{
	...
	if((mes=="｀"||mes=="＾")&&scale!=1.0) x1=x1+3*scale; /* 表示文字が「｀＾」の場合は位置を補正 */
	else if((mes=="｀"||mes=="＾")&&scale==1.0) x1=x1+2*scale; /* 表示文字が「｀＾」の場合は位置を補正 */
	else if((mes=="］"||mes=="［"||mes=="｝"||mes=="｛"||mes=="｜"||mes=="＄")&&scale!=1.0) x1=x1+1*scale; /* 表示文字が「］［｝｛＄」の場合は位置を補正 */
...
function pshiftfinger(g,s,flag) /* シフトキーを表示する */
{
/*   シフトキーの指ガイドを表示 */
/*   s=0 左側のシフトキー */
/*	 s=1 右側のシフトキー */
...
/*   flag= 1 シフトキー消去 */
	...
	x_pos0=keyposit_x(x,y);
	y_pos0=keyposit_y(x,y);
	y_pos1=keyposit_y(x,y+1);
...
function fngposit(finger) /* ポジション練習で指番号を指定 */
{
...
function difposit2(g,yy,flag) /* 指番号に対応したキーを打つ指の爪を表示 */
// yy 指番号
// flag=0 爪を黒で表示
// flag=1 爪を元の色に戻して表示
	{
...
function difposit(g,a,flag) /* 文字に対応したキーを打つ指の爪とシフトキーを打つ指の爪を表示 */
// flag=0 爪を黒で表示
// flag=1 爪を元の色に戻して表示
	{
		var pos1;
...
function dipline2(g,x,line,flag,shift_mode) /* シフトモードに対応してキーボード一列表示*/
// x 表示行位置番号 
// line キートップ文字列 
...
// flag=3 キーの刻印を消去
{
		...
		for(y=y_start;y<ii&&y<y_end;y++)
		{	
			x_pos=keyposit_x(x+1,y+1); /* キーの表示位置の仮想x座標を取得 */
			y_pos=keyposit_y(x+1,y+1); /* キーの表示位置の仮想y座標を取得 */
...
function  errcharpos(a,b) /* エラー文字とガイドキー文字のキーの位置が同じ位置かを判定する */
// =0 一致しない
// =1 一致する
{
	var pos1,pos2;
...
function recovererrchar(a) /* エラー文字位置に表示されていた元の文字を取得 */
{
		var shift_mode;
		var x,y;
		var b;
...
function dikposit(g,a,flag) /* ポジション練習でエラー文字とガイドキー文字の表示及び復帰を行う */
	{
		...
		var aa;
		var pos1;
		var shift_mode,sshift_mode,mshift_mode;
		if(a===0) return(0);
...
function dikposit2(g,a,x,y,flag) /* ポジション練習でエラー文字とガイドキー文字の表示及び復帰を一文字づつ行う */
//	a ガイドキーの文字
...
function dispgrid(g) /* ランダム練習にてグリッドを表示する */
	{
		var i,j,k;
		var x1,x2,y1,y2;
		k=0;
		cslcolor(g,MIKA_green); /* グリッドの色を緑色に設定 */
		g.lineWidth=1; /* 線を表示の場合は太さ1で描画 */
		g.beginPath(); /* 直線描画開始 */
		for(j=0;j<10;j++) /* 最大10行描画 */
		{
			...
			for(i=0;i<80;i=i+2) /* グリッド縦線を40本描画 */
			{
				...
				g.moveTo(y1,x1);
				g.lineTo(y1,x2); /* グリッド縦線描画 */
				k++;
			}
			y1=ycord(0); /* グリッド横線開始y座標を取得 */
			y2=ycord(640-1); /* グリッド横線終了y座標を取得 */
			g.moveTo(y2,x1);
			g.lineTo(y2,x2); /* グリッド最右縦線描画 */
			g.moveTo(y1,x1);
			g.lineTo(y2,x1); /* グリッド上横線描画 */
			g.moveTo(y1,x2);
			g.lineTo(y2,x2); /* グリッド下横線描画 */
...
function proctrainexit(g)/* ランダム練習 の 練習終了時の表示更新 */
	{
...
function proctrain(g,nChar) /* ランダム練習 の文字入力処理 */
{
	...
	else if(MIKA_practice_end_flag==0) /* 練習実行中の場合 */
	{
		...
		if(uppertolower(nChar)==uppertolower(MIKA_key_char)) /* 入力文字と練習文字を小文字に変換して比較 */
		{
			...
			cslputu(g,MIKA_t_line*16+MIKA_c_p2*20,MIKA_c_p1*16,"aa",MIKA_text_underline,MIKA_color_text_under_line); /* 正解文字に下線 */
			if(MIKA_c_p1<39) /* 次の練習文字位置を取得 */
			{
				MIKA_c_p1++; /* 横座標インクリメント */
			}
...
function lowertoupper(nChar) /* 英小文字を英大文字に変換 */
{
	var char1='A';
	var char2='a';
	if('a'<=nChar&&nChar<='z') nChar=String.fromCharCode(nChar.charCodeAt(0)-char2.charCodeAt(0)+char1.charCodeAt(0)); /* 英小文字の場合は大文字に変換 */
	return nChar;
}
...
function getshiftmode(a) /* 練習文字に対応した シフトモードを取得 */
...
function keycord(a) /* 練習文字に対応した キーの位置 列と行を取得 */
{
	...
	for(j=0;j<4;j++)
	{
		i=cfind(a,MIKA_c_post[j]); /* 行ごとにシフト無し 刻印文字列テーブルと一致する文字をサーチ */
...
function convcookie() /* cookie の連想配列データーを成績に変換して格納 */
{
	...
	var i,ii;
	var a;
	if('MIKAKIGO' in MIKA_cookie_array1) /* 連想配列に 'MIKAKIGO'があった場合 */
	{
		a=MIKA_cookie_array1['MIKAKIGO']
